...
async def test_direct_tools():
    ...
    try:
        ...
        print(f"✅ get_company_health: score={res_health.get('score')} label={res_health.get('interpretation')}")
    except Exception as e:
        print(f"ℹ️ get_company_health (offline/stub fallback): {type(e).__name__}")

    # Tool 4: detect_kpi_anomalies
    try:
        res_anom = await asyncio.wait_for(detect_kpi_anomalies(domain="Finance"), timeout=2.0)
    ...
    try:
        res_fc = await asyncio.wait_for(forecast_metric("Revenue", periods=3), timeout=2.0)
        print(f"✅ forecast_metric: metric={res_fc.get('metric')} points={len(res_fc.get('forecast', []))}")
    except Exception as e:
