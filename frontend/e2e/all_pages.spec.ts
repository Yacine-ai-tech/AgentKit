...
const ROUTES = ['/', '/workflow', '/tools', '/intelligence', '/observability', '/resources', '/connect'];

test.describe('AgentKit All Pages E2E Suite', () => {
  ...
  test.beforeEach(async ({ page }) => {
    await page.route('**/*', async route => {
      ...
      if ((req.resourceType() === 'fetch' || req.resourceType() === 'xhr') && url.includes('vercel.app')) {
        let backendUrl = process.env.TEST_BACKEND_URL || 'http://localhost:8002';
