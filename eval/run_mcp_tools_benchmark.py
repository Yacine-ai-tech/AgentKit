        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    result = {"error": f"HTTP {response.status_code}", "detail": response.text}
                    success = False
    ...
    async def run_benchmark(self) -> Dict:
        ...
        for scenario in TEST_SCENARIOS:
            ...
            if success:
                ...
                print(f"  Success: Yes | Time: {exec_time:.2f}s | Memory: {memory_delta:.1f}MB")
                print(f"  Tool Selection: {'✓' if selection_correct else '✗'} | Report Quality: {'✓' if quality_pass else '✗'}")
            else:
        ...
        avg_time = total_time / len(self.results["execution_times"]) if self.results["execution_times"] else 0
        sorted_times = sorted(self.results["execution_times"])
        p95_time = sorted_times[int(len(sorted_times) * 0.95)] if sorted_times else 0
        
        avg_memory = sum(self.results["memory_samples"]) / len(self.results["memory_samples"]) if self.results["memory_samples"] else 0
        max_memory = max(self.results["memory_samples"]) if self.results["memory_samples"] else 0
        
        tool_selection_accuracy = (self.results["tool_selection_correct"] / self.results["total_tests"] * 100) if self.results["total_tests"] > 0 else 0
        execution_success_rate = (self.results["successful"] / self.results["total_tests"] * 100) if self.results["total_tests"] > 0 else 0
        report_quality_rate = (self.results["report_quality"] / self.results["total_tests"] * 100) if self.results["total_tests"] > 0 else 0
        
        print(f"\n=== Aggregate Results ===")
        print(f"Total tests: {self.results['total_tests']}")
        print(f"Successful: {self.results['successful']} ({execution_success_rate:.1f}%)")
        ...
        for category, data in self.results["category_results"].items():
            if data["total"] > 0:
                category_summary[category] = {
                    ...
                    "selection_accuracy": (data["correct_selection"] / data["total"] * 100),
