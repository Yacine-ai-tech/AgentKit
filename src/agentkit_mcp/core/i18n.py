...
The BI services (`insights.py`, `forecasting.py`) were extracted from IntelAI, which is
bilingual (EN/FR). AgentKit is a headless MCP server with no UI, so it runs English-only —
...
class I18N:
    """English-only stand-in for IntelAI's I18N (class-level API: ``I18N.lang()``)."""
